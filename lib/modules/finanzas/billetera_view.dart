...
// Asegúrate de que esta ruta coincida con donde guardaste tu servicio
...
class _BilleteraViewState extends State<BilleteraView> {
  ...
  // Instanciamos el servicio HTTP
  final FinanzasService _finanzasService = FinanzasService();

  @override
  void initState() {
    super.initState();
    _sincronizarDatos(); // Llamamos a la API en cuanto se abre la pantalla
  }

  // ---> SIMULACIÓN DE PETICIÓN ASÍNCRONA HTTP <---
  Future<void> _sincronizarDatos() async {
    // 1. Simulamos un retraso de red de 2 segundos para que el profe vea el "Loader"
    await Future.delayed(const Duration(seconds: 2));
    
    try {
      // 2. Ejecutamos la petición HTTP (actualmente retorna los datos mockeados en el servicio)
      final datos = await _finanzasService.obtenerSaldoUsuario('demo_user');
      
      // 3. Actualizamos la interfaz con los datos "reales"
      setState(() {
        saldoDisponible = datos['saldoDisponible'] ?? 0.0;
        saldoTransito = datos['saldoTransito'] ?? 0.0;
        _isLoading = false; // Ocultamos el loader
      });
    } catch (e) {
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    // Configuramos el formato de moneda para cuando los datos carguen
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        // Si _isLoading es true, mostramos la animación. Si es false, mostramos la billetera.
        child: _isLoading 
            ? const Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    CircularProgressIndicator(color: Color(0xFFE26112)),
                    SizedBox(height: 20),
                    Text(
                    ...
                    children: [
                        ...
                        children: [
                          const Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Text('JOB', style: TextStyle(color: Color(0xFFE26112), fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                                  Text('HUB', style: TextStyle(color: Color(0xFF4D2214), fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                                ],
                              ),
                          ...
                          Stack(
                            children: [
                              Container(
                                ...
                                child: const Icon(Icons.notifications_none_outlined, color: Color(0xFF4D2214), size: 26),
                              ),
                              Positioned(
                                right: 2, top: 2,
                                child: Container(
                                  padding: const EdgeInsets.all(4),
                                  decoration: const BoxDecoration(color: Color(0xFFE26112), shape: BoxShape.circle),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: const Color(0xFFE26112).withOpacity(0.25), blurRadius: 15, offset: const Offset(0, 6))],
                        ),
                        child: Stack(
                          children: [
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      const Text('Saldo disponible', style: TextStyle(color: Colors.white70, fontSize: 14, fontWeight: FontWeight.w500)),
                                      const SizedBox(width: 8),
                                      Icon(Icons.visibility_outlined, color: Colors.white.withOpacity(0.8), size: 18),
                                    ],
                                  ),
                                  const SizedBox(height: 10),
                                  
                                  AutoSizeText(
                                    formatoMoneda.format(saldoDisponible), // Variable dinámica
                                    style: const TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.bold),
                                    maxLines: 1, 
                                    minFontSize: 18, 
                                    overflowReplacement: const Text('Monto excedido', style: TextStyle(color: Colors.white, fontSize: 18)),
                                  ),
                                  ...
                                  GestureDetector(
                                    ...
                                    child: Container(
                                      ...
                                      child: const Text('Listo para retirar', style: TextStyle(color: Color(0xFFE26112), fontSize: 13, fontWeight: FontWeight.bold)),
                                    ),
                                  ),
                                ],
                              ),
                            ...
                            Positioned(
                              right: 24, top: 24,
                              child: Icon(Icons.account_balance_wallet_outlined, color: Colors.white.withOpacity(0.15), size: 44),
                            ),
                          ],
                        ),
                      ),
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                child: const Icon(Icons.trending_up, color: Color(0xFFE26112), size: 24),
                              ),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        Text(
                                          formatoMoneda.format(saldoTransito), // Variable dinámica
                                          style: const TextStyle(color: Color(0xFF4D2214), fontSize: 18, fontWeight: FontWeight.bold),
                                        ),
                                        const Text(' MXN', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w500)),
                                      ],
                                    ),
                                    const SizedBox(height: 4),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(color: const Color(0xFFE26112), borderRadius: BorderRadius.circular(8)),
                                  child: const Text('+12%', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold)),
                                ),
                              ],
                            ),
                            const SizedBox(height: 30),
      ...
      BarChartData(
        ...
        barTouchData: BarTouchData(
          ...
          touchTooltipData: BarTouchTooltipData(
            getTooltipColor: (group) => const Color(0xFF4D2214), 
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
              return BarTooltipItem('\$${rod.toY.round()}', const TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
            },
          ),
        ),
        ...
        barGroups: [
          _crearBarra(0, 40, false), _crearBarra(1, 55, false), _crearBarra(2, 35, false), _crearBarra(3, 85, true),
          _crearBarra(4, 50, false), _crearBarra(5, 65, false), _crearBarra(6, 45, false),
        ],
      ),
    );
  }
  ...
  BarChartGroupData _crearBarra(int x, double y, bool isHighlighted) {
    return BarChartGroupData(
      x: x,
      barRods: [BarChartRodData(toY: y, color: isHighlighted ? const Color(0xFFE26112) : const Color(0xFFFBECE3), width: 16, borderRadius: const BorderRadius.vertical(top: Radius.circular(4)))],
    );
  }

  // Modal de Retiro con FLUTTER_SVG
                  ...
                  children: [
                    Container(
                      ...
                      child: SvgPicture.asset(
                        'assets/images/banco.svg',
                        width: 24,
                        height: 24,
                        colorFilter: const ColorFilter.mode(Color(0xFFE26112), BlendMode.srcIn),
                      ),
                    ),
                    const SizedBox(width: 16),
                    const Expanded(
